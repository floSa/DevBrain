...
- **Trois** activités sans recouvrement partagent ce domaine. **Authentifier** ([[PyJWT]]) est du code qu'on écrit dans son propre service. **Reconnaître** ([[Web-Check]], [[osint4all]]) est une inspection depuis l'extérieur, sans accès privilégié. **Tenir un système qui embarque un modèle** ([[AI security]] et les trois pages qui la détaillent) est une discipline entière, arrivée ici au lot 4 : ce n'est pas un sous-sujet de l'IA générative, c'est une pratique de sécurité qui traverse les modèles.
...
- Comprendre les risques d'une application qui embarque un LLM → [[AI security]].
- Une entrée non fiable qui détourne les instructions → [[Prompt injection]].
- Un contournement de l'alignement du modèle → [[Jailbreaking and defenses]].
- Filtrer, valider ou bloquer autour des appels → [[Guardrails]].
