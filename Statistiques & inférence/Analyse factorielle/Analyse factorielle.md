...
- **Le but est l'interprétation des axes, pas la performance d'un modèle en aval.** C'est ce qui sépare ce dossier de la réduction de dimension du ML — [[t-SNE and UMAP]], [[ICA]], [[NMF]], autoencodeurs —, qui cherche une représentation utile à une tâche et se juge sur elle. Ici on regarde le biplot et on nomme les axes. La frontière est réelle mais fine : les deux familles partagent le tag `dimensionality-reduction`, et [[umap-learn]] vit dans [[Machine Learning]], pas ici.
...
- **Deux branches sortent du tableau rectangulaire.** [[GPA]] superpose plusieurs configurations décrivant les mêmes individus ; [[PGA]] généralise la PCA aux données qui vivent sur une variété courbe (formes, matrices SPD, rotations). [[Manifold learning]] est la branche non linéaire générale — Isomap, LLE, Kernel PCA —, celle qui déplie une variété là où un plan ne suffit pas.
