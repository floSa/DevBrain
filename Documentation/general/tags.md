...
| `automl` | Automatisation de la chaîne de modélisation — sélection d'algorithme, ingénierie de variables et réglage menés par la machine, classement des candidats |
| `data-governance` | Gouvernance de la donnée et des modèles — catalogue, lignage, droits par rôle, journal d'audit, traçabilité de bout en bout |
| `ml-platform` | Plateforme ML de bout en bout — préparation, entraînement, déploiement et gouvernance sous une console unique (distinct de `ml-pipeline`, qui n'orchestre que les étapes) |
