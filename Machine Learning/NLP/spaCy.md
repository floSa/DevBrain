...
- NER **zero-shot** sur des types d'entités arbitraires sans données → [[GLiNER]].
- Fine-tuner un gros transformeur de l'état de l'art pour une tâche unique → [[HuggingFace]].
- Tâches non linguistiques (classification tabulaire, recherche pure) → [[Scikit-Learn]].
...
- NER limité aux **types appris** par le modèle ; pour des types custom, entraîner ou passer à [[GLiNER]].
...
- [[HuggingFace]] — pour le fine-tuning de transformeurs ; complément plus que substitut.
