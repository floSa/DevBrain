...
- **Ce dossier n'est pas « les modèles profonds appliqués aux images » — c'est le critère d'entrée qui range ici.** Le socle qui entraîne le réseau est dans [[Apprentissage profond]] ; un modèle qui *génère* une image à partir d'un texte relève de [[LLM & IA générative]] ; un modèle qui lit une image pour en parler produit du texte, pas une structure — [[Vision Language Models]] est donc rangée avec les autres modèles de fondation, dans [[Modèles de langage]], et son outillage l'a toujours été. Ce qui est ici prend des pixels en entrée et produit une structure : classe, boîte, masque, trajectoire, pose.
