...
- On veut surtout **distribuer une boucle existante** sans gérer la config DeepSpeed → [[accelerate]] (qui sait piloter DeepSpeed) ou le sharding natif **FSDP** de [[PyTorch]].
- On cherche une **structure d'entraînement** complète (Trainer, callbacks) → [[PyTorch Lightning]].
...
- Souvent consommé **indirectement** : `accelerate` et le `Trainer` de [[HuggingFace]] exposent DeepSpeed via une simple config.
...
- [[PyTorch]] — framework sous-jacent (FSDP en est l'équivalent natif de ZeRO-3).
- [[accelerate]], [[HuggingFace]] — exposent DeepSpeed par config.
