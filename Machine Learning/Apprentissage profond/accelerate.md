...
- Socle du fine-tuning `transformers` / `PEFT` ([[HuggingFace]]).

## Quand NE PAS l'utiliser

- On veut une **structure complète** d'entraînement (Trainer, callbacks, logging, checkpointing organisés) → [[PyTorch Lightning]].
...
- S'appuie sur [[PyTorch]] et ses backends distribués (`torch.distributed`, NCCL).
...
- [[PyTorch]] — le framework dont accelerate distribue la boucle (`torch.distributed`, DDP, FSDP).
- [[HuggingFace]] — bibliothèque sœur ; moteur sous le `Trainer` de `transformers`.
- [[datasets]] — source de données de l'entraînement.
