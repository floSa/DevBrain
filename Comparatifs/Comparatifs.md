---
role: hub
nom: Comparatifs
pitch: Ce qui départage plusieurs briques d'un même thème — un tableau par thème, rangé dans le domaine qu'il compare.
---

# Comparatifs

> Ce qui départage plusieurs briques d'un même thème — un tableau par thème, rangé dans le domaine qu'il compare.

## Ce qu'il faut comprendre

- Un comparatif ne décrit aucune brique : il dit **ce qui les sépare**. On l'ouvre quand on hésite entre deux outils du même thème, pas quand on cherche à comprendre l'un d'eux.
- Chaque page embarque une vue `.base` — un tableau qui se remplit seul, filtré sur `categorie:` ou sur un tag — puis une section « Ce qui départage », une puce par membre, écrite à la main. C'est cette section qui porte l'arbitrage ; le tableau ne fait que le préparer.
- **Ce dossier ne porte aucun comparatif, et c'est voulu.** Les 47 pages vivent dans le dossier du domaine qu'elles comparent, à côté des briques qu'elles départagent : c'est ce qui les rend trouvables par `ls`, et la règle de propagation en dépend (`brain-v3.md` §10, ligne P3). Ce hub est le seul endroit qui les réunit toutes — son périmètre est un `role:`, pas un dossier, comme celui des six pages de `Métiers/`.
- La liste ci-dessous est **générée** par `AI/scripts/build_mocs.py` depuis `role: comparatif`. Elle n'est pas tenue à la main : une liste manuelle mentirait au premier comparatif ajouté, et personne ne s'en apercevrait.

## Choisir

- Hésiter entre deux briques d'un même thème → le comparatif du thème, ci-dessous ou dans le hub de son domaine.
- Chercher ce qu'une brique **est**, plutôt que ce qui la distingue de ses voisines → sa fiche, ou la notion de son dossier.
- Chercher un assemblage déjà éprouvé plutôt qu'un arbitrage entre deux briques → [[Patterns]].
- Chercher une contrainte qui tient quelle que soit la stack → [[Rules]].

<!-- AUTO:START -->
### Automatisation no-code
- [[Comparatif - Automatisation no-code]]

### Bases de données
- [[Comparatif - Bases colonnes]]
- [[Comparatif - Bases graphes]]
- [[Comparatif - Bases NoSQL]]
- [[Comparatif - Bases relationnelles]]
- [[Comparatif - Bases temporelles]]
- [[Comparatif - Bases vectorielles]]
- [[Comparatif - Clients de bases de données]]
- [[Comparatif - Migrations de schéma]]
- [[Comparatif - Moteurs de recherche]]
- [[Comparatif - ORM]]

### Calcul distribué
- [[Comparatif - Calcul distribué]]

### Data & pipelines
- [[Comparatif - Manipulation de données]]
- [[Comparatif - Orchestrateurs data]]
- [[Comparatif - Outils EDA - profiling]]
- [[Comparatif - Parsing de documents]]
- [[Comparatif - Scraping]]
- [[Comparatif - Visualisation]]

### Design & diagrammes
- [[Comparatif - Design & prototypage]]
- [[Comparatif - Diagrammes]]

### Interfaces & apps data
- [[Comparatif - Apps data & démos ML]]
- [[Comparatif - Frontends web légers]]

### LLM & IA générative
- [[Comparatif - Assistants de code IA]]
- [[Comparatif - Exécution & serving LLM]]
- [[Comparatif - Fine-tuning LLM]]
- [[Comparatif - Frameworks LLM]]
- [[Comparatif - Frameworks text-to-SQL]]
- [[Comparatif - Observabilité LLM]]
- [[Comparatif - Évaluation LLM]]

### Machine Learning
- [[Comparatif - Boosting]]
- [[Comparatif - Détection & segmentation]]
- [[Comparatif - Détection d'anomalies]]
- [[Comparatif - Explicabilité]]
- [[Comparatif - Forecasting]]
- [[Comparatif - NLP]]
- [[Comparatif - Optimisation d'hyperparamètres]]
- [[Comparatif - Orchestrateurs ML]]
- [[Comparatif - Plateformes data & IA]]
- [[Comparatif - Reinforcement learning]]
- [[Comparatif - Réduction de dimension]]
- [[Comparatif - Serving de modèles]]
- [[Comparatif - Suivi d'expériences ML]]

### Mathématiques
- [[Comparatif - Solveurs d'optimisation]]

### Outils de développement
- [[Comparatif - Clients d'API]]
- [[Comparatif - Frameworks CLI]]
- [[Comparatif - Gestionnaires de paquets Python]]

### Signal & audio
- [[Comparatif - Traitement du signal]]

### Statistiques & inférence
- [[Comparatif - Outils stats]]
<!-- AUTO:END -->
