...
| `status:` | **[!]** 2 `.base` filtrent `status == "actif"` — la page en sort sans bruit · **[!]** cohérence avec `maturite` et `remplace_par` · **[M]** si `abandonne` : mention du successeur dans le corps, car `brain-index.json` **ne porte pas `status:`** et `planifier-projet` continuera donc de proposer la brique | `grep -l 'status == "actif"' Dev/Patterns/*.base` | `sed -n '/^status:/p;/^maturite:/p;/^remplace_par:/p' "Dev/Services/<X>.md"` → trio cohérent ; la section `## Alternatives` de la page nomme un successeur |
