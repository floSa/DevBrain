...
- **Rangement par NATURE, pas par audience** : tout ce qui est **technique** → `Dev/` (jamais `Wiki/`). Brique à déployer (service, framework, lib, BDD) → `Dev/Services/` ; **outil technique** que l'on utilise (client GUI, CLI, utilitaire — DBeaver, pgAdmin, uv…) → `Dev/Outils/` (`galaxie: dev`, `categorie: tooling/<sous-domaine>`). `Wiki/` = **notions** (`Concepts/`) + **skills/extensions de pratique perso** (`Outils/` : Claude Code, Obsidian, MCP). Doute « Dev ou Wiki ? » → est-ce technique ? alors Dev.
