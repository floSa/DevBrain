...
- `Documentation/general/taxonomie.md` — les **deux axes** de rangement : `categorie:` (le domaine) et `famille:` (la nature, 9 valeurs fermées, arbre de décision F1→F9). `type:` ne porte pas la nature.
...
- **Rangement par NATURE, pas par audience** : tout ce qui est **technique** → `Dev/` (jamais `Wiki/`). Brique à déployer (service, framework, lib, BDD) → `Dev/Services/` ; **outil technique** que l'on utilise (client GUI, CLI, utilitaire — DBeaver, pgAdmin, uv…) → `Dev/Outils/` (`galaxie: dev`, `categorie: <domaine>/<sous-domaine>` — **même vocabulaire de 94 valeurs que les Services**, il n'y a plus de préfixe `tooling/` : DBeaver → `database/admin`, uv → `devtools/paquet`). `Wiki/` = **notions** (`Concepts/`) + **skills/extensions de pratique perso** (`Outils/` : Claude Code, Obsidian, MCP). Doute « Dev ou Wiki ? » → est-ce technique ? alors Dev.
...
4. **Vérifier les faits sur le web (d'office)** si un champ factuel change (`maturite`,
   `licence_type`, `status`, `langage`, `url_docs`, `url_repo`). Fin d'étape vérifiable :
...
| `status:` | **[!]** 2 `.base` filtrent `status == "actif"` — la page en sort sans bruit · **[!]** cohérence avec `maturite` et `remplace_par` · **[D]** `status:` est **indexé** depuis le correctif L3-1 : `planifier-projet` écarte d'office une brique `abandonne` · **[M]** renseigner `remplace_par:` — c'est lui qui dit quoi proposer à la place, et il n'est **pas** indexé (4 fiches sur 8 abandonnées le portent) · **[M]** si `abandonne` : mention du successeur dans le corps, pour le lecteur humain | `grep -l 'status == "actif"' Dev/Patterns/*.base` | `sed -n '/^status:/p;/^maturite:/p;/^remplace_par:/p' "Dev/Services/<X>.md"` → trio cohérent ; la section `## Alternatives` de la page nomme un successeur |
| `maturite:` | **[D]** enum fermée · **[!]** `.base` filtrant `maturite != "deprecated"` ou `== "production"` · **[!]** cohérence avec `status` : `deprecated` ⇒ `status != actif` | `grep -l 'maturite' Dev/Patterns/*.base` | `sed -n '/^status:/p;/^maturite:/p' "Dev/Services/<X>.md"` → jamais `deprecated` avec `actif` |
...
| `licence_type:`, `hosted:`, `scaling:`, `langage:` | **[D]** enums fermées, sauf `langage` · lus en direct par les vues `.base`, rien à propager | — | `uv run AI/scripts/check_brain.py` |
