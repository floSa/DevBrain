...
- Conversation, rédaction, raisonnement : hors périmètre → [[Ollama]], [[llama.cpp]].
- Sortie structurée sur un modèle plus gros et plus capable → [[Outlines]], [[Instructor]].
- Extraction d'entités nommées en zero-shot → [[GLiNER]].
...
- L'outil de synthèse de données de fine-tuning appelle [[OpenRouter]] par défaut (`OPENROUTER_API_KEY`) : dépendance cloud pour l'entraînement, pas pour l'inférence.
