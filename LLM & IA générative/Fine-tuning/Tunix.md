...
Bibliothèque Google de **post-training de LLM en [[JAX]]** (*Tune-in-JAX*), construite sur Flax NNX : [[SFT]] (full weights ou PEFT), alignement par préférences (DPO, ORPO), RL ([[GRPO]], PPO et variantes GSPO-Token, DAPO, Dr.GRPO, jusqu'au **RL agentique** multi-tour avec appels d'outils) et **distillation**. Les rollouts d'inférence s'appuient nativement sur [[vLLM]] ou SGLang-JAX. Pensée pour la performance sur **TPU** et le passage à l'échelle — c'est le pendant JAX/TPU de [[TRL]] (qui est PyTorch).
...
- Écosystème **PyTorch / Hugging Face** en place → [[TRL]] : intégration native `transformers`/`peft`, communauté et recettes incomparablement plus larges.
- Fine-tuning sur **un GPU grand public** → [[Unsloth]] (kernels optimisés VRAM), pas une lib pensée TPU.
...
- [[TRL]] — le voisin PyTorch ; même rôle, autre écosystème.
- [[JAX]] — le socle de calcul (Flax NNX).
- [[vLLM]] — moteur de rollouts pour le RL.
