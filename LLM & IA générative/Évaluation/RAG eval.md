...
- **Reference-free** : métriques calculées par [[Routing and cascading|un autre LLM]] juge (LLM-as-judge), sans réponse gold — c'est l'approche de [[Ragas]].
...
### La triade RAG
- Formalisée par [[TruLens]] : *context relevance*, *groundedness*, *answer relevance* — un triangle qui couvre retrieval + génération.
...
- Même esprit au niveau **claim** : [[Ragas]] décompose déjà la faithfulness en affirmations ; RAGChecker pousse la logique pour **attribuer** chaque erreur au retriever ou au générateur.
...
- Outiller : [[Ragas]] (référence open-source, sans référence), [[DeepEval]] (assertions façon pytest, en CI), [[TruLens]] (tracing + feedback functions). Pour le diagnostic fin : **RAGChecker** (attribution retriever vs générateur au niveau claim), **ARES** (juges légers calibrés sur ~150 annotations humaines).
...
- [[Ragas]], [[DeepEval]], [[TruLens]] — l'outillage dédié.
