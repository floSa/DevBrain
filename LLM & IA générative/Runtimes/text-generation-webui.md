...
Interface web (bâtie sur [[Gradio]]) pour faire tourner des LLM en local, par **oobabooga** — le « couteau suisse » historique du milieu. Sa force : le **multi-backends commutables sans redémarrage** — [[llama.cpp]], ik_llama.cpp, Transformers, ExLlamaV3 et [[TensorRT-LLM]] — ce qui permet de charger à peu près n'importe quel format de modèle (GGUF, GPTQ, EXL2…). Chat, mode notebook, **vision**, **tool-calling**, extensions, et une **API compatible OpenAI/Anthropic** (Chat/Completions/Messages). Désormais distribué aussi comme **app de bureau** (rebrand « TextGen »). Écrit en **Python**, licence **AGPL-3.0**.
...
- Démarrage ultra-simple en une commande → [[Ollama]].
- GUI de bureau packagée et soignée → [[LM Studio]].
...
- UI construite sur [[Gradio]] ; modèles tirés du hub [[HuggingFace]].
- Peut piloter [[llama.cpp]] et [[TensorRT-LLM]] comme backends.
